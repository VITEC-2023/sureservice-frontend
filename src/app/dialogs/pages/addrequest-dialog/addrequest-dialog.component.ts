...
export class AddrequestDialogComponent implements OnInit {
  ...
  createData(){
    ...
    this.newProfileEService.createRequest(this.getCurrentclientId(),this.getCurrentemployeeId(),this.request).subscribe( (response: any) => {
    })
    this.created=true;
  ...
  getCurrentitemData(){
    let currentItemData= localStorage.getItem('requestData')
    if(currentItemData){
